...
class AddPage(BasePage):
    ...
    def should_be_add_product_to_cart_batton(self):
        ...
        assert self.is_element_present (*AddToCartLocators.ADD_BUTTON), "Batton is not presented"

    def should_be_product_add_to_cart_message(self):
        # проверка, что есть сообщение, что товар добавлен в корзину.
        assert self.is_element_present (*AddToCartLocators.ADD_PRODUCT_MESSAGE), "Message is not presented"
    ...
    def should_be_product_name_is_name_in_message(self):
        ...
        name_product = self.give_element_present(*AddToCartLocators.NAME_PRODUCT)
        message_add_product= self.give_element_present(*AddToCartLocators.ADD_PRODUCT_MESSAGE)
    ...
    def should_be_cost_cart_message(self):
        ...
        assert self.is_element_present (*AddToCartLocators.COST_CART_MESSAGE), "The cost of the basket does not match the price of the goods"
		
    def should_be_cost_cart_coincides_price_product(self):
        # проверка, что стоимость корзины совпадает с ценой товара.
        cost_cart = self.give_element_present(*AddToCartLocators.COST_CART)
        price_product= self.give_element_present(*AddToCartLocators.PRICE_PRODUCT)
        assert cost_cart in price_product, f"Стоимость корзины '{cost_cart}' с ценой товара '{price_product}' не совпадает"